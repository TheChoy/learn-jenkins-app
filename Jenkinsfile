pipeline {
    ...
    stages {
        ...
        stage('Build Project') {
            ...
            steps {
                script {
                    echo "🏗️ กำลังก่อสร้างโปรเจค..."
                    try {
                        sh '''
                        set -x  # เพิ่มการแสดงคำสั่งที่รันใน log
        ...
        stage('Deploy to Netlify') {
            ...
            steps {
                script {
                    echo "🚀 กำลังกำหนดการ deploy ไปที่ Netlify..."
                    try {
                        sh '''
        ...
        stage('Monitor Server Resources') {
            ...
            steps {
                script {
                    echo "🔍 กำลังกำหนดการตรวจสอบทรัพยากรเซิร์ฟเวอร์หลังจากการ deploy..."
                    try {
                        sh '''
